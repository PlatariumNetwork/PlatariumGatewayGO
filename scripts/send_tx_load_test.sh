#!/usr/bin/env bash
# Send transactions 1→2 and 2→1 with statistics (second console).
# Before running, start run_100nodes.sh in the first console (or at least one testnet node on 2812/2813).
#
# Usage:
#   cd PlatariumGatewayGO && ./scripts/send_tx_load_test.sh [ROUNDS]
#   ROUNDS - number of TX pairs (1→2, 2→1); default 20.
...
ROUNDS="${1:-20}"
# Number of nodes in the network - TX sent round-robin to different nodes to test propagation
NODES="${SEND_TX_NODES:-100}"
BASE_PORT="${BASE_PORT:-2812}"
BASE_URL="${BASE_URL:-http://localhost:$BASE_PORT}"   # used only for "Waiting for gateway" (node 0)
SAMPLE_NODES="${SAMPLE_NODES:-0,10,50,99}"   # node indices for balance checks (port = BASE_PORT+10*i)
...
# Parse generate-mnemonic output: Mnemonic: ... Alphanumeric: ...
...
# Parse sign-message: Public Key and Compact (first Main Signature block). Output: two lines (pubkey, compact).
parse_sign_output() {
  local out=$1
  ...
  # compact: first 128 hex chars only (Core expects compact)
...
echo "=== Platarium Load Test: TX 1→2, 2→1 (network distribution) ==="
echo "Nodes:       $NODES (TX round-robin: each TX to the next node)"
...
# Wait until at least one node is ready
...
# Count consecutive responding nodes from 0 - round-robin only over them (nodes 0,1,...,NODES-1)
...
for (( i=0; i<100; i++ )); do
  ...
  if [ "$code" = "200" ]; then
    ...
    [ -z "$code" ] && code="timeout/refused"
    echo "  Node $i (port $port): down ($code)"
    break
  fi
done
if [ "$detected" -eq 0 ]; then
  echo "  ERROR: No nodes responding. Start gateway: ./scripts/run_100nodes.sh or a single node on port $BASE_PORT."
  exit 1
fi
NODES=$detected
echo "  - Summary: $NODES node(s) (ports $BASE_PORT..$((BASE_PORT+10*(NODES-1)))). TX round-robin over these only."
[ "$NODES" -eq 1 ] && echo "  (Single node - all TX go to it. For distribution test run: ./scripts/run_100nodes.sh)"
echo ""

# Canonical field order (matches CoreVerifyMessage)
build_msg() {
  local from=$1 to=$2 value=$3 nonce=$4 ts=$5
  echo "{\"from\":\"$from\",\"to\":\"$to\",\"value\":\"$value\",\"nonce\":$nonce,\"timestamp\":$ts,\"type\":\"transfer\"}"
}

# $1 = node URL (http://localhost:PORT), rest = TX parameters
send_tx() {
...
for (( r=0; r<ROUNDS; r++ )); do
  ...
  # Progress every 5 rounds and on the first round
...
echo ""
echo "========== Statistics =========="
echo "  Transactions 1→2:  OK=$ok_1to2  (via fallback to node 0: $ok_fallback_1to2)  FAIL=$fail_1to2"
echo "  Transactions 2→1:  OK=$ok_2to1  (via fallback to node 0: $ok_fallback_2to1)  FAIL=$fail_2to1"
echo "  Total success:     $total_ok"
[ "$total_fallback" -gt 0 ] && echo "  Success via fallback (node did not respond - sent to node 0): $total_fallback"
...
if command -v bc &>/dev/null && [ -n "$elapsed" ] && [ "$(echo "$elapsed > 0" | bc)" -eq 1 ]; then
  ...
  echo "  TX/s (successful): $tps"
fi
echo "================================="

# Propagation check: balances on several nodes
echo ""
echo "Propagation check (balances on different nodes):"
